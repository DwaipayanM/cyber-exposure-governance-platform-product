...
def inject_theme() -> None:
    st.markdown(
        ...
        /* ---- Tabs: dark band with white text, green active pill (matches header) ---- */
        .stTabs [data-baseweb="tab-list"] {
            gap:4px; margin-top:10px; padding:7px; border-bottom:none; flex-wrap:wrap;
            background:linear-gradient(120deg,#15171E 0%,#1D1F27 55%,#15315C 130%);
        ...
        [data-testid="stSidebar"] button[data-testid="stBaseButton-secondary"] {
            ...
            box-shadow: 0 2px 5px rgba(1, 169, 130, 0.1) !important;
        }
        [data-testid="stSidebar"] button[data-testid="stBaseButton-secondary"]:hover {
            color: #ffffff !important;
            background-color: #01A982 !important;
            border-color: #01A982 !important;
            box-shadow: 0 4px 10px rgba(1, 169, 130, 0.2) !important;
        }
...
with tabs[0]:
    ...
    st.dataframe(styled(df[cols].head(10)), use_container_width=True, hide_index=True)
...
with tabs[3]:
    ...
    with c[1]:
        ...
        st.dataframe(styled(ids_view), use_container_width=True, hide_index=True)
...
with tabs[4]:
    ...
    with c[1]:
        ...
        st.dataframe(priv_view, use_container_width=True, hide_index=True)
...
with tabs[5]:
    ...
    st.dataframe(styled(df[[c for c in gov_cols if c in df.columns]]), use_container_width=True, hide_index=True)
...
with tabs[6]:
    ...
    st.dataframe(styled(ds), use_container_width=True, hide_index=True)
...
with tabs[8]:
    ...
    st.dataframe(audit_df.tail(100), use_container_width=True, hide_index=True)
    ...
    if snapshots.empty:
        ...
        st.dataframe(snapshots.tail(20), use_container_width=True, hide_index=True)
        if len(snapshots) > 1:
